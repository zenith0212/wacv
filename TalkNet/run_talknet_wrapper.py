...
def visualization(tracks, scores, args):
	...
	for fidx, fname in tqdm.tqdm(enumerate(flist), total = len(flist)):
		image = cv2.imread(fname)
		framewise[int(os.path.basename(fname)[:-4])] = []
		for face in faces[fidx]:
			box = [int(face['x']-face['s'])/fw, int(face['y']-face['s'])/fh,\
					int(face['x']+face['s'])/fw, int(face['y']+face['s'])/fh]
			framewise[int(os.path.basename(fname)[:-4])].append([box, face['score']])
		...
		vOut.write(image)
	vOut.release()
	save_file = open(os.path.join(args.pyaviPath, 'face_conf.pkl'), 'wb')
	pickle.dump(framewise, save_file)
	save_file.close()
